...
categories:
  - 文档翻译
excerpt: psutil (python版的系统和进程实用包) 是一个跨平台库, 可用于在Python程序中检索正在运行的进程和系统利用率(CPU, 内存, 磁盘, 网络, 设备)的相关信息。该包实现了由Unix命令行工具提供的许多功能，例如:ps, top, lsof, netstat, ifconfig, who, df, kill, free, nice, ionice, iostat, iotop, uptime, pidof, tty, taskset, pmap.
---
